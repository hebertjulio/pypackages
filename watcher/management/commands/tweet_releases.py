...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for language in ['python', 'javascript', 'css']:
            ...
            if twitter_account is not None:
                auth = tweepy.OAuthHandler(twitter_account["API_KEY"], twitter_account["API_SECRET"])
                auth.set_access_token(twitter_account["ACCESS_TOKEN"], twitter_account["ACCESS_TOKEN_SECRET"])
    ...
    def write_tweet(self, release):
        try:
            ...
            api.update_status(self.tweet_message.format(release.package.name, release.name))
